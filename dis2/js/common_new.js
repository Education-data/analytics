...
$('.js_podpiska').click(function (e) {
    e.preventDefault();
    check_form_podpiska(this);
    return false;
});
 ...
 function check_form_podpiska(target) {
     
     var email = $('form.header_form input').val();
     var email_ = /^[\w-\.]+@[\w-]+\.[a-z]{2,4}$/i;
   
    var name_test = email_.test(email);
    
    if (!name_test){
        $('form.header_form input').css('border','1px solid red');
    }else{
        $('form.header_form input').val('');
        $('form.header_form input').css('border','1px solid #fff');
        $('form.header_form button').html('Подписка оформлена!');
    }
 }
